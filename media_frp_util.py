...
def setup_frp_and_keep_alive(port=8000, frp_config=None):
    """
    Setup FRP tunnel for remote access to the media server.
    
    Args:
        local_port (int): Local port where HTTP media server is running
        frp_config (str): Path to FRP configuration file
    
    Returns:
        subprocess.Popen: FRP process handle
    """
...
def setup_frp(port=8000):
    if not download_frp():
        print("Error: Failed to download frpc.exe")
        return

    import threading
    frp_thread = threading.Thread(target=setup_frp_and_keep_alive, args=(port,))
    frp_thread.start()